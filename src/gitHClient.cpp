...
Client::Client(const QString Grepo, const quint16 pNumber, const QString Gtoken, QObject *parent)
    : QObject(parent), m_Grepo(Grepo), m_pNumber(pNumber), m_Gtoken(Gtoken), m_apiAddress("https://api.github.com"),
...
void Client::getDiff()
{
    ...
    request.setRawHeader(QByteArray("Authorization"), QByteArray("Bearer ").append(m_Gtoken.toUtf8()));
    request.setRawHeader(QByteArray("Accept"), QByteArray("application/vnd.github.diff"));
...
void Client::sendReview(const QJsonObject &payload)
{
    ...
    QObject::connect(reply, &QNetworkReply::finished, this, [=]() { reply->deleteLater(); });
