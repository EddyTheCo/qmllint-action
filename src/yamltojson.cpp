...
size_t offsetToLine(const size_t &offset,
                    const std::vector<size_t> &linewidths) {
  ...
  if (file.exists() && file.open(QIODevice::ReadOnly)) {
    ...
    if (diagnostics && diagnostics.IsSequence()) {
      ...
      for (const auto &diag : diagnostics) {
        ...
        const auto filePath =
            QString::fromStdString(diagMess["FilePath"].as<std::string>());
        const auto message =
            QString::fromStdString(diagMess["Message"].as<std::string>());
        const auto fileOffset = diagMess["FileOffset"].as<size_t>();
        files[filePath].push_back(QJsonObject(
            {{"message", message}, {"charOffset", int(fileOffset)}}));
      }
      const auto json = QJsonDocument(addLines(files));
      QFile jsonFile(fileInfo.absoluteFilePath().replace(".cpp.o.yaml",
                                                         "_clang-tidy.json"));
...
int main(int argc, char *argv[]) {
  ...
  parser.setApplicationDescription(
      "Create json lint files from clang-tidy yaml files");
  parser.addHelpOption();

  parser.addPositionalArgument(
      "rPath",
      QCoreApplication::translate("main", "Absolute path of the yaml files"));
  ...
  const QStringList files =
      filePath.entryList(QStringList() << "*.yaml", QDir::Files);
