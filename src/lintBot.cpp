...
int main(int argc, char *argv[])
{
    ...
    parser.setApplicationDescription("Create reviews for a PR using json lint files");
    parser.addHelpOption();

    parser
        .addPositionalArgument("jPath",
                               QCoreApplication::translate("main",
                                                           "Absolute path to the lint json files"));
    parser.addPositionalArgument(
        "rPath", QCoreApplication::translate("main", "Absolute path of the local github repo"));
    parser.addPositionalArgument("GRepo", QCoreApplication::translate("main", "OWNER/REPO"));
    parser.addPositionalArgument("PRNumber",
                                 QCoreApplication::translate("main", "Pull Request Number"));
    ...
    QObject::connect(gitHclient, &Client::ready, &app, [=, &app]() {
        ...
        const QStringList files = jsonPath.entryList(QStringList() << ("*.json"), QDir::Files);
